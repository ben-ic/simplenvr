...
Modeled on Frigate's published architecture. Every component except the ONNX model is pure Python + OpenCV + existing SimpleNVR infra — no new dependencies, no new RTSP connections, no new ffmpeg pipes.
...
**3. Classifier worker (new, ~350 lines total)**. One shared ONNX Runtime session for all 32 cameras. Consumes `tracked_event` messages from the bounded async queue. For each tracked event:
  ...
  - Collapses COCO class ids → `{person, vehicle, animal, None}` **at the scoring layer**, not post-hoc — same labelmap approach Frigate documents
...
5. Battle-tested in Frigate's production deployment (Frigate also uses onnxruntime for its ONNX detector path — we are not inventing)
...
Same library Frigate uses under a thin provider-selection wrapper. Execution provider chain, picked at startup in priority order:
...
The product ships to grandma's Costco laptop *and* your Snapdragon X Elite from the same installer. Hard-coding a model choice doesn't work. Instead, SimpleNVR runs a **one-time calibration benchmark** at first launch and picks the tier from measured latency, not from a spec database.
...
| Camera | Scene | Baseline full-frame | **Frigate-style motion crop** | v1 behavior |
|---|---|---|---|---|
...
1. **The Frigate-style motion-blob crop is the single biggest quality win.** Taking a car from "not detected" to "vehicle 0.58 via car" on the same frame with the same model, just different preprocessing. This is not a model-selection problem — it's a pipeline problem.
...
**Cost-per-event budgets must use the tracked-event rate, not the raw-trigger rate.** Any design that bills per raw motion frame is economically broken. Frigate bills per tracked event, which is why their Frigate+ service is economically viable.
...
- **Object mask UI** — user-drawn rectangles to exclude specific screen regions from any detection. Frigate's final-resort fix for persistent false positives that MOG2 background subtraction doesn't catch. Ship after v1 proves stable.

---

## v2 (later)

- **Package detection via YOLOX fine-tuned on Open Images V7**. The `Box` class exists in Open Images (annotations CC-BY 4.0 Google, images CC-BY 2.0 Flickr — commercial fine-tuning and shipping the weights is permitted). Cloud L4/A10 fine-tune, ~$50–$200, ~1–2 weeks work. COCO has no package class; this is the only license-clean path that doesn't route through Frigate+'s closed model.

- **Delivery logo second-stage classifier** (Amazon / UPS / FedEx / DHL) — Frigate+'s differentiating feature. A tiny MobileNet/EfficientNet-Lite on ~300×300 crops of detected packages. User-facing sentence: *"Amazon package at Front door."*
...
- `frigate_style.py` — 4 tile strategies + label collapse
...
Re-running the experiment when daytime recordings are available is the remaining empirical step — expect confident ≥0.60 person/vehicle detections on daytime frames via Frigate-style motion-blob crops, validating the full pipeline end-to-end before code is written in `backend/classification/`.
