...
Commit `261475e` ("Fall back to sub-stream when main stream fails") tried to fix a Tapo C120 UDP-loop by wrapping `authed_uri()` with a helper that appended `?rtsp_transport=tcp`. The commit's own comment claimed it only affected the go2rtc registration path, but `authed_uri()` is also called by the periodic `verify_rtsp_uri` probe in `backend/discovery/scanner.py::_probe_due_uris`, which runs every `URI_PROBE_INTERVAL` (5 minutes). Within one interval every Reolink in the test rig flipped to `needs_auth`, the recorder's circuit breaker tripped into sub-fallback, and the UI asked for passwords that were already correct. The commit was reverted in `cca8383`.

The next attempt at forcing TCP for Tapo-class cameras must keep `?rtsp_transport=tcp` **out** of `authed_uri()`'s return value. The only paths allowed to append it are go2rtc sources (where it's a recognized go2rtc source option, parsed and stripped before the upstream DESCRIBE). ffmpeg's CLI already gets `-rtsp_transport tcp` as a flag in `backend/recording/codec.py::build_unified_cmd`, so the recorder's direct-camera fallback path does not need URL-level TCP forcing either. If a future fix adds a `go2rtc_source_url(camera)` helper that applies the query param, it must be a separate function — not a layer on top of `authed_uri` — and any test for it must assert that `authed_uri` output has an empty query string.
