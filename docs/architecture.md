...
- **`rtsp_probe.py`** — two jobs: (1) verifies a stored RTSP URI is actually reachable (stale-camera detection), and (2) `scan_rtsp_devices` actively probes a set of hosts for an open RTSP port and identifies them from the RTSP `Server:` banner. The active scan catches cameras with ONVIF disabled and is the only discovery mechanism that works across subnets (see `networks.py`). Probe fan-out is bounded by `MAX_CONCURRENT_PROBES` so a multi-subnet sweep can't exhaust file descriptors.
- **`networks.py`** — resolves *which* hosts the active RTSP sweep covers. Unions every directly-connected interface subnet (enumerated via `psutil.net_if_addrs`, deduped, loopback/link-local excluded) with the user's authorized off-link CIDRs from `Settings.scan_networks`. See "Multi-network discovery" below.
...
- **`reconcile.py`** — tiered DHCP-rebind reconciliation. When an endpoint appears at an IP no DB row owns, `narrow()` checks offline candidates by unauthenticated signals only: exact EndpointReference (EPR_EXACT → reconcile, no auth), primary-MAC + ≥2 corroborations from ONVIF scopes / RTSP banner / OUI brand (MAC_HIGH → reconcile, no auth), or primary-MAC + 1 corroboration / alt-MAC match (MAC_MODERATE → single verify-auth call against the singleton prime suspect, matching HardwareId + SerialNumber confirms). Any tie or zero-signal miss refuses to reconcile — silent mis-reconcile is strictly worse than a stranded offline row. The load-bearing architectural rule: **auth is a verifier, not a matcher**. A successful auth at a new IP proves the stored credentials work there, *not* that the device is any particular DB row — households routinely share one admin password across every camera they own, so a credential-stuff strategy would silently mis-attribute recordings to whichever candidate happened to accept the same creds (a correctness bug far worse than a visible stranded offline row). Identity has to come from unauthenticated, device-intrinsic signals (EPR, MAC + corroborations); auth is only ever used to *confirm* a singleton candidate that unauthenticated evidence has already identified. Speculative auth against candidates also costs camera-side lockout risk and log noise for zero identity gain. The scanner therefore makes at most one auth call per reconcile event, only in the MAC_MODERATE branch, only against the singleton prime suspect — credentials are never iterated across candidates to identify a device. Full rationale: `plans/mac-fallback-ip-change-plan.md` §Architecture.
...
**Multi-network discovery — `networks.py`.** ONVIF WS-Discovery is multicast, which is link-local: routers don't forward `239.255.255.250:3702` across subnet boundaries, so a camera on a different VLAN or a remote site is invisible to it. The active RTSP sweep (`rtsp_probe.scan_rtsp_devices`) is unicast and *can* cross subnets — the only missing piece was telling it which hosts to probe. `networks.resolve_scan_plan()` builds that host list from two sources:

- **Directly-connected interface subnets** — every IPv4 NIC the host owns (via `psutil.net_if_addrs`), deduped across NICs, loopback and link-local skipped. These are *always* in scope: the machine is a member of those networks, so probing them needs no extra consent. A multi-homed box (camera VLAN on one NIC, office LAN on another) now scans both with zero configuration — previously only the primary interface's /24 was swept.
- **User-authorized off-link CIDRs** — `Settings.scan_networks`, a list of CIDR/host strings the user explicitly adds in Settings. This is the consent gate for networks the host can *route to* but is not a member of (another subnet, a remote site over VPN). We never probe a routed range we weren't told to. The "cameras in multiple networks of the same org" case is exactly this: list each site's camera subnet and they're all discovered.

**Safety rails (load-bearing — an active port scan is the one place SimpleNVR reaches beyond the box).** An authorized CIDR may not exceed `MAX_AUTHORIZED_HOSTS` (1024, ~/22) — a fat-fingered `/8` is rejected at the API with a 400 listing the reason, not silently swept. Interface subnets that resolve absurdly wide are skipped. The combined host list across all networks is capped at `MAX_SCAN_HOSTS` (4096); if the cap clips it, `ScanPlan.truncated` is True and the scanner logs that some hosts won't be probed (no silent truncation). Hosts are deduped at the IP level, so an authorized CIDR overlapping an interface subnet — or a bare host inside its own /24 — never double-probes. `GET /api/settings/scan-networks` returns the resolved plan (interface subnets + authorized + rejected entries + total host count) so the Settings UI can show the user exactly what will be scanned before they rely on it. The plan is reloaded every scan pass, so adding a network takes effect on the next sweep without a restart (mirrors the declared-brands hint).

**Profile selection ranks codec before area.** `onvif_client.select_main_and_sub` picks main/sub from `GetProfiles` by `(codec_rank, area)` — H.265 beats H.264 beats MJPEG, with area breaking ties within the same codec rank. A naive area-only selector would tiebreak on ONVIF response order, and some cameras (Tapo C120) return an MJPEG "preview" profile first at the same 640×360 resolution as the real H.264 sub — writing an MJPEG URI into `substream_uri` and producing black recordings the first time the chronic-failure breaker downgrades to sub. The codec tag is captured into `cameras.substream_codec` / `cameras.rtsp_codec` so the recorder can branch on it at spawn. Full rationale: `plans/substream-codec-aware-plan.md` §Architecture.
...
- **`codec.py`** — builds the unified ffmpeg command line. One ffmpeg instance, two outputs: (1) stream-copy to disk segments, (2) scene-filtered JPEG frames at ~1 fps that feed the recorder's `motion_broadcaster`. In v2 those JPEGs no longer drive detection — the AudioManager uses them for event thumbnails and `/api/cameras/{id}/snapshot` serves the `.latest` cache for live-view previews. See Motion + Detect-frames subsystems for the v2 detection path. Live preview is NOT an ffmpeg output — it's rendered natively by libmpv reading from go2rtc's RTSP loopback. See "Live preview path" below. `build_unified_cmd` is also codec-aware: H.264 and H.265 sources stream-copy (the default, zero CPU); MJPEG sources transcode to H.264 via the platform hardware encoder at record time, because stream-copying MJPEG into the fragmented MP4 container produces one fragment per frame and every downstream player renders the result as black. On a platform without a hardware encoder the builder raises `UnsupportedSourceCodecError` and the recorder surfaces `camera_health="unsupported_codec"` instead of silently producing unplayable files. Full rationale: `plans/substream-codec-aware-plan.md`.
